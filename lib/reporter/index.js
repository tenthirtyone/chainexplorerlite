  ...
  cacheHasBlockData(startBlock, endBlock) {
    ...
    while (current <= endBlock) {
      if (!this.sharedCache.exists(current)) {
        this.logger.info("Blocks NOT found in cache");
        return false;
      }
      current++;
    }

    this.logger.info("Blocks found in cache");
    return true;
  }

  getBlocksFromInfura(startBlock, endBlock) {
    let current = startBlock;
    while (current <= endBlock) {
      if (!this.sharedCache.exists(current)) {
        this.logger.info(`Adding Block ${current} to Infura queue`);
        this.infura.addBlockToQueue(current);
      }
      current++;
    }
  }
